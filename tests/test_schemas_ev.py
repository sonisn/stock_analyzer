...
def test_expected_return_is_probability_weighted_sum():
    ...
    pick = _pick(
        [
            Scenario(label="bull", probability=0.30, target_return_pct=60.0, rationale="ai capex"),
            Scenario(label="base", probability=0.50, target_return_pct=8.0, rationale="steady"),
            Scenario(
                label="bear", probability=0.20, target_return_pct=-25.0, rationale="recession"
            ),
        ]
    )
...
def test_returns_none_when_probabilities_dont_sum_to_one():
    ...
    pick = _pick(
        [
            Scenario(label="bull", probability=0.20, target_return_pct=60.0, rationale="x"),
            Scenario(label="base", probability=0.30, target_return_pct=8.0, rationale="x"),
            Scenario(label="bear", probability=0.20, target_return_pct=-25.0, rationale="x"),
        ]
    )
