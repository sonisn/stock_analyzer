"""The rebalancer's system prompt, one piece per section.

Static sections are constants; sections that template CC/CSP settings from
`.env` are functions. `_build_rebalancer_instructions` joins them in order.
"""

from __future__ import annotations

# Role, the long-term mandate, and what the user provides.
_ROLE_AND_INPUTS = """\
...
# Default to no action; the triggers that justify one; aggressiveness modes.
_WHEN_TO_ACT = """\
...
# When the tax bar does not apply: tax-advantaged accounts, loss positions.
_TAX_BAR_OVERRIDES = """\
...
# Deployment order, account placement, where sale proceeds go.
_DEPLOYING_MONEY = """\
...
# Data-only reasoning, continuity across runs, hard constraints.
_CONSTRAINTS = """\
...
# Wash-sale rules and tax-loss harvest candidates.
...
# The two allowed prose formats (no action / rebalance plan).
_OUTPUT_FORMAT = """\
...
# Critical rules and the anti-hallucination citation rule.
_CRITICAL_RULES = """\
CRITICAL:
...
# The structured fields and their agreement with the prose.
_STRUCTURED_OUTPUT = """\
STRUCTURED OUTPUT:
...
def _covered_call_writing(
    *, cc_target_delta_min: float, cc_target_delta_max: float, cc_dte_min: int, cc_dte_max: int
) -> str:
    """Covered-call writing band, strike choice and liquidity guard."""
    return f"""\
========================================================================
...
def _premium_reinvestment(
    *, cc_slippage_buffer: float, cc_min_premium_usd: float, buffer_pct: int
) -> str:
    """Deploying expected premium net of the slippage buffer."""
    return f"""\
========================================================================
...
def _cash_secured_puts(
    *,
    csp_target_delta_min: float,
    csp_target_delta_max: float,
    csp_dte_min: int,
    csp_dte_max: int,
    csp_max_pct_per_put: float,
    csp_max_pct_total: float,
) -> str:
    """Cash-secured puts: posture, caps, and the csp_writes entry."""
    return f"""\
========================================================================
...
def _stub_consolidation(*, cc_min_stub_usd: float) -> str:
    """Stub consolidation: selling sub-100-share stubs to complete round lots."""
    return f"""
========================================================================
